...
testParallel = do
    Just doc <- currentDocument
    Just body <- getBody doc
    result <- uncheckedCastTo HTMLDivElement <$> createElement doc "div"
    appendChild_ body result

    setInnerHTML result "Here comes the result"

    let p n = threadDelay (n * 1000000) >> return n
    runStream $ do
        n <- parallely $ p 3 |: p 2 |: p 1 |: S.nil
        setInnerHTML result (show n)
-}
